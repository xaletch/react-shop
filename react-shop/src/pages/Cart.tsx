...
const Cart: React.FC = () => {
  ...
  return (
    <div className="container container--cart">
      ...
      {clearCart && (
        <div className='clear-cart' onClick={() => setClearCart(false)}>
          <div className='clear-cart_menu cart--empty' onClick={(e) => e.stopPropagation()}>
            <p>Вы действительно хотите очистить корзину?</p>
            <div className='clear-cart_btn'>
              <button className='clear-cart_btn-no' onClick={() => setClearCart(false)}><span>Нет</span></button>
              <button onClick={handleClear}><span>Да</span></button>
            </div>
          </div>
        </div>
      )}
